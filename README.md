...
By default an `Agent` has file tools (read/write/edit/list/find, confined to `workdir`),
memory tools, a permission engine in `AUTO` mode, a redacting audit log at
`workdir/audit.jsonl`, and crash-resumable sessions. Everything is overridable.
...
```python
from engine.mcp import StdioMCPConnection
...
```python
from engine import HostAllowlist, Mode
...
| Capability | Where |
| :--- | :--- |
| Tool-argument validation against `input_schema` | `engine/validation.py` |
| Secret redaction before hashing | `engine/audit/redact.py` |
...
| MCP stdio client, namespaced + fault-isolated | `engine/mcp/` |
...
```bash
python -m pytest        # 210 tests, offline (no API key, no network)
```

`tests/test_hardening.py` pins the fixes for defects that were reproduced against the
